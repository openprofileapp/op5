...
export default function Presense({ 
    ...
    const presenseClassList = 
        `absolute rounded-full ${largeIcons ? "h-8 w-8 border-6" : "h-6 w-6 border-4"} bottom-0 right-0 border-base-100 z-2 tooltip tooltip-top`;
    ...
    switch (data?.presence) {
        ...
        case "idle":
            return (
                ...
                >
                    <svg className="relative top-0.25 left-0.25" width={largeIcons ? "19" : "15"} height={largeIcons ? "19" : "15"} viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
        ...
        case "dnd":
            return (
                ...
                >
                    <div className={`${largeIcons ? "text-lg" : "text-base"} text-base-100 font-nerdfont leading-none w-full text-center`}>
                        󱘹
                    </div>
                </div>
            );
        case "offline":
            return (
                <div className={`${presenseClassList} bg-base-300`}>
                    <div className="tooltip-content">
                        <div className="font-bold">{t("words.LastActive")}</div>
                        <div className="text-xs">{formatLongRelative(data?.lastActive as string)}</div>
                    </div>
                </div>
            );
        default:
